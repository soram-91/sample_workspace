...
public class MethodExample01 {
	...
	// 메소드 : 입력값 -> 처리 -> 출력값.
	public static void sum(int n1, int n2) { // void : 반환 유형이 없습니다. return x
	...
	public static int sum2(int n1, int n2) {
		int result = n1 * 2 + n2 * 3;
		return result; // sum2 메소드를 호출한 영역으로 결과값을 반환.
	}

	// 키 입력 -> 처리 -> 적정 몸무게 반환. 적정 몸무게 : (키 - 100)* 0.9
	public static double properWeight(double height) {
		double weight = (height - 100) * 0.9;
		return weight;
	}
	
	// 삼각형의 넓이 메소드 (밑변, 높이) => 넓이 / 밑변*높이/2 넓이: getTriangle() 
